...
export function InteractiveMap({
  ...
  const [locallyFocusedVolunteerId, setLocallyFocusedVolunteerId] = useState<string | null>(null)
  ...
  useEffect(() => {
    if (seniorsProp && volunteersProp && assignmentsProp && clustersProp) {
      ...
      const volunteersWithAssignments = volunteersProp.filter((volunteer) => {
        return assignmentsProp.some((assignment: any) => {
          ...
          const assignmentDate = new Date(
            assignment.date || assignment.scheduled_date || new Date().toISOString().split("T")[0],
          )
      ...
      const processedClusters = clustersProp.map((cluster: any) => {
        const centroid =
          typeof cluster.centroid === "string" ? JSON.parse(cluster.centroid) : cluster.centroid || cluster.center
        ...
        const clusterSeniors = schedulesFromAssignments
          ...
          .flatMap((schedule) =>
            schedule.seniors.map((sid) => filteredSeniors.find((s) => s.uid === sid)).filter((s): s is Senior => !!s),
          )
    ...
    const initializeMap = async () => {
      try {
        ...
        map.current.on("load", () => {
          ...
          const beforeLayer =
            layerIds.find((id) => id.includes("building") || id.includes("road") || id.includes("label")) || undefined
        ...
        setMapError(
          error instanceof Error ? error.message : "Failed to initialize map. Please check your Mapbox configuration.",
        )
      }
    }
  ...
  const createSeniorPopupHTML = (senior: Senior, priority: "HIGH" | "MEDIUM" | "LOW") => {
    const lastVisit = senior.last_visit ? new Date(senior.last_visit).toLocaleDateString() : "Never visited"
  ...
  return (
    <div className="w-full h-[600px] relative overflow-hidden">
      ...
      {mapError && (
        <div className="absolute inset-0 flex items-center justify-center bg-muted z-10">
          <div className="text-center p-4 max-w-md">
            <p className="text-destructive font-medium">Map Configuration Required:</p>
      ...
      {/* Legend */}
      <div className="absolute bottom-4 left-4 bg-white p-3 rounded-lg shadow-lg" style={{ zIndex: 10 }}>
