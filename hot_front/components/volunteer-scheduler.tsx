...
import { Loader2, Calendar, Clock, MapPin, User, CheckCircle2 } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
    ...
    try {
      ...
      if (email !== "") {
        ...
        const userAssignments = assignmentsData.assignments
          .filter(
            (assignment: { volunteer_email: string; date: string }) =>
              assignment.volunteer_email === email && weekDays.includes(assignment.date),
          )
          .map(
            (assignment: {
              aid?: string
              ...
              const senior = seniorsData.seniors.find((s: { uid: string }) => s.uid === assignment.sid)
              return {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:8000"}/acknowledgements`,
        {
          method: "PUT",
          headers: {
      ...
      <CardHeader className="pb-0">
        <div className="flex items-start justify-between">
          ...
          <Badge
            ...
            className={isNew ? "bg-red-50 text-red-700 border-red-200" : "bg-green-50 text-green-700 border-green-200"}
          >
            {isNew ? (
              "New"
            ) : (
      ...
      <CardContent className="py-2">
        <div className="space-y-3">
          ...
          <div className="flex items-start gap-2">
            ...
            <p className="text-sm text-gray-700 leading-relaxed">{assignment.address}</p>
          </div>
        </div>
      </CardContent>
      {isNew && (
          ...
          <div className="w-full">
            <div className="flex items-center justify-between">
              <div className="flex flex-col justify-center">
                <span className="text-sm font-semibold text-blue-800 mb-1">Please acknowledge this assignment</span>
              </div>
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span className="text-sm font-medium text-blue-700">Acknowledge</span>
              </label>
            </div>
          </div>
        </CardFooter>
  ...
  const renderAssignmentSection = (
    ...
    <div className="w-full mx-auto space-y-4">
      <div className="flex items-center justify-center gap-2 mb-6">
        <Badge variant={isNew ? "destructive" : "secondary"} className="text-base px-4 py-2">
          {badgeText}
        </Badge>
      </div>
      {assignments.length > 0 ? (
        assignments
          ...
          .map((assignment) => <AssignmentCard key={assignment.aid} assignment={assignment} isNew={isNew} />)
      ) : (
      ...
      {isLoading ? (
        ...
        <>
          {renderAssignmentSection(
            "New Assignments",
            "New",
            userSchedule.filter((s: UserSchedule) => s.is_acknowledged === false),
            true,
          ...
          {renderAssignmentSection(
            ...
            "Acknowledged",
            userSchedule.filter((s: UserSchedule) => s.is_acknowledged === true),
            false,
